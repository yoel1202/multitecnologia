...
$consulta =$conexion->consulta("SELECT id_orden,numero_orden FROM tbl_orden_compra where id_orden='".$_GET['listaorden']."' ");
...
echo '<a  href="pdf/ordencompra.php?listaorden='.$_GET['listaorden'].'"  text="Orden de compra" class="btn btn-sm btn-primary btn-block" > Ver orden</a> ';
 ...
 if (isset($_GET['listaorden'])) {
   # code...
    ...
    $consulta =$conexion->consulta("SELECT id_lista,tp.nombre,numero_orden,lo.cantidad,precio,tp.unidad FROM  tbl_lista_orden lo INNER JOIN tbl_productos tp on tp.id_producto=lo.fk_producto INNER JOIN tbl_orden_compra  on id_orden=fk_orden  where fk_orden='".$_GET['listaorden']."'");
