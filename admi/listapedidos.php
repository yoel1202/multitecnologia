...
<!-- /navbar -->
 ...
 $tamano=10;
    ...
    $consulta =$conexion->consulta("SELECT id_pedido,TC.nombre,TP.cantidad,TP.hora,TP.observacion,TP.fecha,CONCAT(TH.nombre,' ',TH.cedula),TP.estado FROM  tbl_comidas TC INNER JOIN tbl_pedidos TP ON TP.fk_comida=TC.id_comida INNER JOIN tbl_clientes TH ON TH.idl_cliente = TP.fk_cliente where  TP.estado='PROCESANDO'");
$numero_filas = $consulta->num_rows;
             $total_paginas =ceil( $numero_filas / $tamano);
             if (isset($_GET['pagina'])) {
              $pagina=$_GET['pagina'];
              $pagina_inicio=($pagina-1)*$tamano;
                  $consulta =$conexion->consulta("SELECT id_pedido,TC.nombre,TP.cantidad,TP.hora,TP.observacion,TP.fecha,CONCAT(TH.nombre,' ',TH.cedula),TP.estado FROM  tbl_comidas TC INNER JOIN tbl_pedidos TP ON TP.fk_comida=TC.id_comida INNER JOIN tbl_clientes TH ON TH.idl_cliente = TP.fk_cliente where  TP.estado='PROCESANDO' LIMIT $pagina_inicio,$tamano ");
               }else{
      $consulta =$conexion->consulta("SELECT id_pedido,TC.nombre,TP.cantidad,TP.hora,TP.observacion,TP.fecha,CONCAT(TH.nombre,' ',TH.cedula),TP.estado FROM  tbl_comidas TC INNER JOIN tbl_pedidos TP ON TP.fk_comida=TC.id_comida INNER JOIN tbl_clientes TH ON TH.idl_cliente = TP.fk_cliente where  TP.estado='PROCESANDO' LIMIT 0,$tamano ");
                   }  
                     if (isset($consulta)) {
